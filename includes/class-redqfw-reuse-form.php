...
class Red_Rb_Reuse {
	...
	public static function load($localPath) {
        $reuseform_scripts = json_decode(file_get_contents( REDQ_RB_DIR . "/resource/reuse.json"),true);
	  ...
	  wp_localize_script( 'reusejs', 'REUSE_ADMIN', array(
          'LANG'          => Red_Rb_Reuse::reuse_form_language(),
          'ERROR_MESSAGE'	=> Red_Rb_Reuse::reuse_form_error_messages(),
          '_WEBPACK_PUBLIC_PATH_' => $localPath,
          'base_url' => apply_filters( 'reuse_image_base_url',  REDQ_RB_DIR ),
        ));
	}
